...
`陨石导弹(Meteor Missile,高破坏性)`、`暴雪导弹(Blizzard Missile)`、
...
- **暴雪导弹**:无论落点处于哪个群系,都会强制进入暴雪天气 5 分钟,并且不受群系温度限制,
  可以正常积雪。暴露在室外的玩家会周期性受到冰冻伤害,每秒累计损失 1% 移动速度(最高叠加到
  100%),并且暴露时间越长,血量上限也会逐渐降低。室内判定是严格的空气连通性检测(站在树下
  不算室内);进入室内后,已积累的负面效果会随时间缓慢恢复,而不是立刻消失。
- **樱花导弹**:没有任何负面效果。落点处于哪个群系都会持续 5 分钟飘落原版樱花花瓣,并可以
  正常堆积在地面;暴露在花瓣雨中的玩家会逐渐清除身上的负面效果,并且每秒回复 2 颗心生命值。
- 暴雪/浓雾的负面效果与樱花的增益效果都会像原版药水效果一样,在效果 HUD 和物品栏效果列表中
  显示各自的图标。所有天气类负面效果对创造模式和旁观模式玩家均不生效。
